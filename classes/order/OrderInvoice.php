...
class OrderInvoiceCore extends ObjectModel
{
    ...
    /** Funcion getAddressShop modificada para Tiendas de Bluecommerce
     * Luis C.
     */

    public static function getAddressBluecommerce($id_shop = null){
        $formataddress = []; //Nombre - Direccion 1 
        $formatadress2 = []; //Codigo Postal Ciudad
        ...
        array_push($formataddress,$address->company,$address->address1);
        array_push($formatadress2,$address->postcode,$address->city);

        $cadenashop = implode("<br>",$formataddress);
        $cadenashop .= '<br>'.implode("",$formatadress2);
        $cadenashop .= '<br>'.$address->phone;
        return $cadenashop;

        // Formateo directo sin string desde array
        //return $formataddress[0].'<br>'.$formataddress[1].'<br>'.$formataddress[2].$formataddress[3].'<br>'.$formataddress[4];
